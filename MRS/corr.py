...
def baseline_correct(G):
    ...
    baseidx = []
    baseidx.extend(range(np.min(np.where(G.f_ppm < 5.0)),
                         np.max(np.where(G.f_ppm > 4.0))+1))
    baseidx.extend(range(np.min(np.where(G.f_ppm < 3.5)),
                         np.max(np.where(G.f_ppm > 3.2)) + 1))
    baseidx.extend(range(np.min(np.where(G.f_ppm < 2.8)),
                         np.max(np.where(G.f_ppm > 2.5))+1))
    ...
    # spline
    f = interpolate.interp1d(adjbaseppm[::-1], yArr[::-1], kind='linear',
